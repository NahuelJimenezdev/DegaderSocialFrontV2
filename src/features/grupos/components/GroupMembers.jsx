...
const GroupMembers = ({ groupData, refetch, user, userRole, isAdmin, isOwner }) => {
  ...
  const handleTransferOwnership = async (newOwnerId) => {
    if (
      window.confirm(
        '¿Estás seguro de que quieres transferir la propiedad de este grupo? Perderás todos los privilegios de propietario.'
      )
    ) {
      try {
        setLoading(true);
  ...
  const handleRemoveMember = async (memberId, memberName) => {
    if (
      window.confirm(
        `¿Estás seguro de que quieres expulsar a ${memberName} del grupo?`
      )
    ) {
      try {
        setLoading(true);
        ...
        await refetch();
        setShowRoleMenu(null);
        alert('Miembro expulsado exitosamente');
      } catch (err) {
